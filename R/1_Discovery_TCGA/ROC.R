...
rocobj <- roc(rt$fustat,rt$Age)
ggroc(rocobj, alpha = 0.5, colour = "royalblue", linetype = 1, size = 1.5)+theme_minimal()
graph2ppt(file="effect plot.pptx", width=7, height=6)

basicplot=ggplot(rt,aes(d=fustat,m=Age,col='red'))+geom_roc(n.cuts = 0)
#pdf('plotROC.pdf',width = 10,height = 8)
basicplot+style_roc()
dev.off()

#styleplot          #??ROC
roc=roc(rt$fustat,rt$Age,plot=FALSE,ci=TRUE)
